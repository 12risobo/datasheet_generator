...
class TestContentBuilder:
    ...
    def test_group_specs_by_section(self, sample_specs_data):
        ...
        assert fiber_section[0][0] == "Fiber type"
        assert fiber_section[0][1] == "Multimode"
    ...
    def test_create_product_info_section(self, sample_product_data):
        ...
        # Find tables in the story
        tables = [elem for elem in story_elements if isinstance(elem, Table) or 
                 (isinstance(elem, KeepTogether) and any(isinstance(item, Table) for item in elem.flowables))]
